...
describe('concat', () => {
  it('should concat "abc" and "de"', () => {
    const result = concat("abc", "de");
    expect(result).to.equal("abcde");
  });

  it('should concat "a" and "a"', () => {
    const result = concat("a", "a");
    expect(result).to.equal("aa");
  });

  it('should throw an error if either of the strings are empty', () => {
    expect(() => concat("", "a")).to.throw('empty string provided');
    expect(() => concat("a", "")).to.throw('empty string provided');
    expect(() => concat("", "")).to.throw('empty string provided');
  });
});